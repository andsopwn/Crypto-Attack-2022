...
#define DIR "C:\\Users\\louxsoen\\Documents\\��ä�ο���\\AES CPA\\"
...
void M(u8 S[16])
{
	...
	for (int i = 0; i < 16; i += 4) // temp�� ���� ���� : mixcolumns�� state ������ �ս�
	{
...
int main()
{
	...
	u8		temp[34];
	u8		iv, hw_iv; // �������� �عֿ�����
	u8		R10[16] = { 0xf2, 0x98, 0x3f, 0x40, 0x30, 0xed, 0xf6, 0x33, 0x04, 0x91, 0x10, 0x3f, 0xd1, 0xb1, 0x3f, 0xa1 };	 // ������ ������Ű
	u8		R9[16] = { 0x73, 0x8D, 0x34, 0x43, 0xC2, 0x75, 0xC9, 0x73, 0x34, 0x7C, 0xE6, 0x0C, 0xD5, 0x20, 0x2F, 0x9E };
	u8		R9S[16];
	double	maxCorr; // ������ �ִ밪(PEAK���� ��)
	double* corr;	// �ڷ����̼� ��
	double	HW;	  // �عֿ���Ʈ�� ��, ���·��� ��
	double	HW_2, * WT_2; // �عֿ���Ʈ �������� ��, ���·��� �������� ��
	double* hw_wt;		// �ع� x ������ ��
	double* WT; // ���� ���°����� ��, ���°��� ������ ��
	double	a, b, c;
	float** WT_data;  // �ĵ��� ��ü ������ ������
	...
	rfp = fopen(buf, "r"); // read binary�� �ƴ�. ������ r
	if (rfp == NULL)
		printf("%s ���� �б� ����", ctFN);

	CT = (u8**)calloc(TraceNum, sizeof(u8*)); // ������ : �� �־��ָ� ����ȭ
	for (i = 0; i < TraceNum; i++)
		CT[i] = (u8*)calloc(16, sizeof(u8));

	CT2 = (u8**)calloc(TraceNum, sizeof(u8*)); // ������ : �� �־��ָ� ����ȭ
	for (i = 0; i < TraceNum; i++)
