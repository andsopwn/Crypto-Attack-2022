...
void CPA(int TEVEC0, int TEVEC1)
{
	float		**data		= NULL;
	u8			**PT 		= NULL;
	u8			iv, hw_iv; 
...
//==============================================================//
	...
	for(i = 0 ; i < TraceNum ; i++)
	{
		...
		for(j = S1_makeM ; j < E1_makeM ; j++) {
			for(k = S2_KeyMask ; k < E2_KeyMask ; k++)
			cut[i][m + (k - S2_KeyMask)] = (data[i][k] - mean[k]) * (data[i][j] - mean[j]);
		}
		m += E2_KeyMask - S2_KeyMask;
	}
//==============================================================//
//						New data CPA							//
//==============================================================//
	...
	{	
		...
		for (key = 0; key < 256; key++) {
			...
			printf("\r%02dth Block : %.1lf%% CR[%lf] K[%02X], TE0[%d] TE1[%d]", i, ((double)key / 255) * 100, maxCorr, maxkey, E2_KeyMask, E3_RMASK);
