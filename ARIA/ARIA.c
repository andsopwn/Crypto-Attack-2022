...
int ENC_KeySchedule(const u8 *MK, u8 *RK, int keysize)
{
	...
	for(i = 0 ; i < 16 ; i++)   T[i] = S[i % 4][KRK[q][i] ^ MK[i]];
	DiffLayer(T, W1);
...
void ARIA()
{
    ...
    //u8      MK[32] = { 0xCB, 0xF6 , 0xD7, 0x4C, 0x0B, 0x78, 0xB9, 0x30, 0x22, 0xB4, 0x7E, 0xAA, 0x06, 0xE7, 0x01, 0x2A };  // MasterKey
    ...
    //ror(s0, 50); ror(s1, 50); ror(s2, 50); ror(s3, 50); // s0~s3은 h0~s3의 50bits 수 만큼 right shift
    //for(int i = 0 ; i < 16 ; i++) printf("%02X ", s0[i] ^ h0[i]); puts("");
    //for(int i = 0 ; i < 16 ; i++) printf("%02X ", s1[i] ^ h1[i]); puts("");
    //for(int i = 0 ; i < 16 ; i++) printf("%02X ", s2[i] ^ h2[i]); puts("");
    //for(int i = 0 ; i < 16 ; i++) printf("%02X ", s3[i] ^ h3[i]); puts("");
